...
- **`src/lib/db.ts` + `src/lib/studio.ts` + `src/lib/ai.ts`** — the data-access layer: thin typed wrappers around `invoke(...)` (page CRUD in `db.ts`; Studio documents/projects in `studio.ts`; AI settings/plan/apply in `ai.ts`) plus the shared `Page`/`StudioDocument`/`StudioProject` types and `convertFileSrc` helpers. Nearly all `invoke` calls live here; the lone exception is a direct `invoke("show_character_palette")` in `PageEditor.tsx`. Add new backend calls as a typed function here, not inline in a component.
...
- **`src/lib/ai.ts`** is the AI data-access layer (provider is OpenRouter only: `AI_PROVIDER_OPENROUTER`). The frontend never calls the model directly — it asks the backend to plan, then to apply: `generate_ai_action_plan` returns a structured `AiActionPlan` (a versioned list of `create_page` / `create_subpages` / `create_database` / `create_database_rows` actions), the user previews it, then `apply_ai_action_plan` executes it against the same page/DB tables. Add new AI action variants to the `AiAction` union here and mirror them in `ai.rs`.
- The API key lives backend-side: it is set via `save_ai_api_key` / cleared via `clear_ai_api_key` and is **never returned to the frontend** — settings only expose a `has_api_key` boolean. Don't surface or log the raw key.
...
- **`src/lib/aiChat.ts`** holds the pure chat-message helpers; **`src/components/AiActionModal.tsx`** is the plan-preview UI. Co-located tests: `ai.test.ts`, `aiChat.test.ts`, and e2e `tests/e2e/ai.e2e.ts`.
