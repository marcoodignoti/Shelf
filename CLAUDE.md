...
npm run check          # Full gate: build + unit + e2e + npm audit + cargo fmt/clippy/test
```
...
- **`src-tauri/src/lib.rs`** — the entire Rust backend in one file: SQLite setup, schema, and every `#[tauri::command]`. The schema evolves via idempotent `CREATE TABLE IF NOT EXISTS` + best-effort `ALTER TABLE ADD COLUMN` blocks at startup (there is no migration framework — add new columns the same way). Tables: `pages`, `studio_documents`, `studio_projects`, `app_metadata`. DB path: `~/Library/Application Support/org.opennotion.desktop/opennotion.db`.
