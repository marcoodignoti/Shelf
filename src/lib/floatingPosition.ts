...
export function computeFloatingPosition(
  ...
  const effectiveWidth = Math.min(floating.width, maxWidth);
  ...
  return {
    left: Math.max(margin, Math.min(preferredLeft, viewport.width - effectiveWidth - margin)),
    top: Math.max(margin, Math.min(preferredTop, viewport.height - effectiveHeight - margin)),
    maxWidth,
    maxHeight,
  };
}
