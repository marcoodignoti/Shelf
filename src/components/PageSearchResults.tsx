...
export function PageSearchResults({
  ...
  // Compute each page's absolute (0-based, across all sections) index for highlight + mouse tracking.
  let runningIndex = 0;
  return (
    <>
      {sections.map((section) => {
        const startIndex = runningIndex;
        runningIndex += section.pages.length;
        return (
          <div key={section.titleKey} className="on-command-section">
            ...
            {section.pages.map((page, withinSectionIndex) => {
              const absoluteIndex = startIndex + withinSectionIndex;
              ...
              return (
                <button
                  ...
                  className={`on-command-item ${isDisabled ? "opacity-40 cursor-not-allowed" : ""} ${isSelected ? "on-command-item-selected" : ""}`}
                  disabled={isDisabled}
